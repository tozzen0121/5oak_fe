...
const Breadcrumbs = ({
    ...
    if (!custom && menu.children) {
      menu.children.filter((collapse) => {
        if (collapse.type && collapse.type === 'collapse') {
          ...
          if (customLocation === collapse?.url || customLocation.includes(collapse?.urlLike)) {
            setMain(menu);
            setItem(collapse);
          }
        }
  ...
  // items
  if ((item && item.type === 'item') || (item?.type === 'group' && item?.url) || (item?.type === 'hidden' && (item?.url || item?.urlLike)) || custom) {
    ...
    let tempContent = (
      <MuiBreadcrumbs aria-label="breadcrumb" maxItems={maxItems || 8} separator={separatorIcon}>
        <Typography component={Link} to="/admin/home" color="textSecondary" variant="h6" sx={{ textDecoration: 'none' }}>
          {icons && <HomeOutlined style={iconSX} />}
          {icon && !icons && <HomeFilled style={{ ...iconSX, marginRight: 0 }} />}
          {(!icon || icons) && 'Admin'}
        </Typography>
